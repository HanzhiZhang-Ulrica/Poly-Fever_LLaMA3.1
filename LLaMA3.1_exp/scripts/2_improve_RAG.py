...
# File paths
lda_model_path = 'LDA/LDA.model'
tfidf_model_path = 'LDA/tfidf.model'
rag_data_path = 'rag/datasets'
data_file_path = '../data/Poly-FEVER_all.tsv'
record_file_path = 'RAG_LLaMA3.1_8B_record.tsv'
output_file_path = 'RAG_LLaMA3.1_8B_output.tsv'
...
# Nuance topics
nuance_topics = [0, 2, 4, 5, 8, 11, 12, 14, 16, 20]
...
def process_row(row, temp_file_path):
    ...
    en_claim = claims[0]
    topics = lda.lda_get_topics([en_claim], tfidf_model_path, lda_model_path)
    try:
    ...
    with torch.no_grad():
        for i, claim in enumerate(claims):
            ...
            # Translate the claim into English if it is nuance topics
            if language != 'en' and sorted_topics[0][0] in nuance_topics:
                claim = GoogleTranslator(source=language, target='en').translate(claim)
                constrain = constrain_translations['en']
            else:
                constrain = constrain_translations[language]

            try:
                response_label = rag(constrain, claim, rag_data_path)
                if "Error code" in response_label:
                    raise Exception(response_label)
